...
describe('Robot', () => {
  ...
  it('test data 5 result should be 3', () => {
    ...
    it('should throw an error if steps is not an integer', () => {
        const request = test_data_set_1 as Request;
        const robot = new Robot(request.start.x, request.start.y);
        expect(() => {
            robot.move('N', 1.5);
        }).toThrowError('Invalid steps');
    });

    it('should throw an error if steps is less than 0', () => {
        const request = test_data_set_1 as Request;
        const robot = new Robot(request.start.x, request.start.y);
        expect(() => {
            robot.move('N', -1);
    ...
    it('should throw an error if steps is greater than 100000', () => {
        const request = test_data_set_1 as Request;
        const robot = new Robot(request.start.x, request.start.y);
        expect(() => {
            robot.move('N', 100001);
    ...
    it('should throw an error if x is less than -100000', () => {
      const request = test_data_set_1 as Request;
      const robot = new Robot(-100001, 0);
      expect(() => { robot.move('north', 1) }).toThrowError('Invalid position');
    });

    it('should throw an error if x is greater than 100000', () => {
      const request = test_data_set_1 as Request;
      const robot = new Robot(100001, 0);
      expect(() => { robot.move('north', 1) }).toThrowError('Invalid position');
    });

    it('should throw an error if y is less than -100000', () => {
      const request = test_data_set_1 as Request;
      const robot = new Robot(0, -100001);
      expect(() => { robot.move('north', 1) }).toThrowError('Invalid position');
    });

    it('should throw an error if y is greater than 100000', () => {
      const request = test_data_set_1 as Request;
      const robot = new Robot(0, 100001);
      expect(() => { robot.move('north', 1) }).toThrowError('Invalid position');
    });
